...
{
	public static IDictionary<string, int> Calculate(IEnumerable<SmartLabel> getReceiveLabels, IEnumerable<SmartLabel> getReceiveAddressLabels, IEnumerable<SmartLabel> getChangeAddressLabels, IEnumerable<SmartLabel> getTransactionLabels, Intent intent)
	{
		var labelPool = new Dictionary<string, int>(); // int: score.

		// Make recent and receive labels count more for the current wallet
		var multiplier = 100;
		foreach (var label in getReceiveLabels.Reverse().SelectMany(x => x.Labels))
		{
		...
		foreach (var label in getReceiveAddressLabels.SelectMany(x => x.Labels))
		{
		...
		foreach (var label in getChangeAddressLabels.SelectMany(x => x.Labels))
		{
			var score = 1;
			if (!labelPool.TryAdd(label, score))
			{
		...
		foreach (var label in getTransactionLabels.SelectMany(x => x.Labels).Reverse())
		{
	...
	public static IEnumerable<SuggestionLabelViewModel> GetLabels(IEnumerable<SmartLabel> getReceiveLabels, IEnumerable<SmartLabel> getReceiveAddressLabels, IEnumerable<SmartLabel> getChangeAddressLabels, IEnumerable<SmartLabel> getTransactionLabels, Intent intent)
	{
		var labelPool = new Dictionary<string, int>(); // int: score.
		...
		return mostUsedLabels.Select(x => new SuggestionLabelViewModel(x.Label, x.Score));
	}
}
